...
ejecutarTestCifradoMasFrecuente = runTestTT testCifradoMasFrecuente
...
ejecutarTodosLosDescifrados = runTestTT testTodosLosDescifrados
testTodosLosDescifrados = test [
...
    ]
    
ejecutarDescifrarVigenere = runTestTT testDescifrarVigenere
testDescifrarVigenere = test [
    "Caso 1: el resultado tiene q ser el mismo" ~: descifrarVigenere "hola" "a" ~?= "hola",
    "Caso 2: clave más larga que el mensaje" ~: cifrarVigenere "hpnd" "abcdabcd" ~?= "hola", -- 
    "Caso 3: mensaje vacio" ~: cifrarVigenere "" "clave" ~?= "",
    "Caso 4: cifrado que rota toda la palabra" ~: cifrarVigenere "gnkz" "zzzz" ~?= "hola", --no se si vale la pena
    "Caso 5: test del TP" ~: cifrarVigenere "kdueciirqdv" "ip" ~?= "computacion",
    "Caso 6: palabra sin minúsculas" ~: cifrarVigenere "COMPUTACION" "ip" ~?= "COMPUTACION",
    "Caso 7: palabra con mayúsculas y minúsculas" ~: cifrarVigenere "kOMecTirqON" "ip" ~?= "cOMpuTaciON"
